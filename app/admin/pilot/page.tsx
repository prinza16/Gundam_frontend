...
const PilotList: React.FC = () => {
  ...
  const fetchPilot = useCallback(
    async (pageToFetch: number = currentPage, currentSearchQuery: string) => {
      ...
      setError(null);
      try {
        const url = new URL(`http://127.0.0.1:8000/pilot/`);
  ...
  useEffect(() => {
    fetchPilot(1, debouncedSearchQuery);
  }, [debouncedSearchQuery, fetchPilot]);
  ...
  const handleCloseDeleteModal = () => {
    ...
    setPilotToDeleteId(null);
  };

  const confirmDeletePilot = async () => {
    if (pilotToDeleteId === null) return;
    setLoading(true);
    setError(null);
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/pilot/${pilotToDeleteId}/`,
        {
          method: "PATCH",
          headers: {
      ...
      const currentItemInPage = pilots.length;
  ...
  const handlePageChange = (pageNumber: number) => {
    if (pageNumber > 0 && pageNumber <= totalPages) {
      fetchPilot(pageNumber, debouncedSearchQuery);
    }
  };
    ...
    return (
      <div className="w-full flex flex-col min-h-[calc(100vh-100px)] bg-gray-900 text-blue-100 p-6 rounded-lg shadow-x">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-blue-200">Pilot</h1>
              ...
              <tbody className="divide-y divide-blue-700">
                {pilots.map((pilot, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3 whitespace-nowrap text-xl font-medium text-blue-200 text-center">
                      {pilot.pilot_universe_name}
                    </td>
                    <td className="px-6 py-3 whitespace-nowrap text-xl font-medium text-blue-200 text-center">
                      {pilot.pilot_images ? (
                        <img
                          src={pilot.pilot_images}
                          alt={pilot.pilot_images}
                    ...
                    <td className="px-6 py-3 whitespace-nowrap text-xl font-medium text-blue-200 flex items-center justify-center">
                      ...
                      <button
                        ...
                        onClick={() => handleOpenDeleteModal(pilot.pilot_id)}
                      >
